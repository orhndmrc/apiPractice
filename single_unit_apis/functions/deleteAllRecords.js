...
/**
 * cleans all record submissions
 * @param {*} token 
 */
let deleteAllRecord = function(token){
    let response, redirectUrl, json;
    response = http.del("https://qa-practical.qa.swimlane.io/api/app/aF5sqnNFCc36kO9_J/record",
    "",
        {
            "cookies": {
    ...
    if(!check(response, {
        "delete all record returns 204": response => response.status.toString() === "204",
      })){
          let checker = {}
